...
内部状态：
- 平台：知乎
- 状态：draft-r2，已完成事实 / 图片核验与独立读者审阅，待用户终审
...
想体验 ROCm、运行 PyTorch，或者临时找一块 AMD GPU 跑模型，不一定要先准备本地显卡和完整的软件环境。我这次从 AMD AI 开发者计划兑换了 1 小时云算力，在 Radeon Cloud 创建 Notebook，确认 PyTorch 已识别 AMD GPU，并用 Qwen2.5-0.5B-Instruct 完成了一次推理。

### 经验值怎样变成 Radeon Cloud Credits

AMD AI 开发者计划（ADP）首页显示的是“经验值”，旁边注明“1 积分对应 1 经验值”。因此，页面其他位置所说的积分，对应的就是这里看到的经验值。

![ADP 当前经验值](images/01-adp-experience.png)

实际兑换关系是：ADP 经验值 / 积分先变成云算力券，云算力券再充值为 Radeon Cloud Credits。本文从账号已经拥有可兑换经验值开始；经验值可以通过 ADP 的活动和课程获得。

### 用 ADP 积分领取云算力券

登录 AMD AI 开发者计划，进入“我的权益”，可以看到“积分免费兑换云额度”。

![积分兑换云额度入口](images/02-adp-benefit.png)

打开[积分兑换页面](https://developer.amd.com.cn/points/redeem)后，页面会列出当前规则。2026 年 8 月 13 日的规则是：1 积分兑换 1 小时算力，每个账号每天最多兑换 20 积分；ADP 积分本身不过期，云算力券有效期为 30 天，充值到 Radeon Cloud 后的云算力有效期为 90 天。
...
这段链接就是云算力券，进入 Radeon Cloud 后会作为 `Coupon Code` 使用。它是不记名虚拟卡券，遗失后不会补发，也不要把完整链接放进文章、截图或公开聊天记录中。
...
Profile 写明：每块 GPU 每小时消耗 1 Credit，每个账号只能保留一个活动实例。这里的 Credit 会在启动实例时预先扣除，不是等运行满一小时后再扣款。

本次单 GPU 实例只运行了约 4 分钟，页面已经显示 `Credits Consumed` 为 1；提前销毁后，未使用的分钟不会退回。因此，点击 `Launch` 前最好先准备好要执行的代码和模型。
...
进入 Terminal 后，我先用下面的命令确认 PyTorch 是否真的看到了 AMD GPU：
...
输出中的 `Model device` 为 `cuda:0`，模型也正常返回了回答。模型已经跑通，但此时云端实例仍处于活动状态，最后还需要手动销毁。
