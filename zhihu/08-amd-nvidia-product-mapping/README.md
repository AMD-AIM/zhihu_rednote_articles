...
|---|---:|---|---:|---|
| RTX 5070 Ti | 16 GB GDDR7 | RX 9070 XT | 16 GB GDDR6 | 2025 高端，容量相同，N 卡官方首发起价高 $150 |
| RTX 5070 | 12 GB GDDR7 | RX 9070 | 16 GB GDDR6 | 同周上市，美国官方首发起价都是 $549 |
...
有个容易忽略的事：RTX 5070 只有 12 GB，但对面 RX 9070 有 16 GB。按美国官方首发起价，两张卡同为 $549，A 卡多了 4 GB——有时候跑大模型的时候这 4 GB 决定你能不能多装一个尺寸。另外， 9060 XT 和 5060 Ti 都有 8 GB / 16 GB 两个版本，买卡和看别人的经验时记得看容量，型号一样不代表显存一样。
...
R9700（32 GB）：单卡跑通了 Qwen3.6-35B-A3B 的 Q4 量化版，也跑过 Qwen2.5-0.5B 的 FP16 未量化推理。前者在本项目这次单卡配置下成功运行；实际余量仍取决于具体量化文件、上下文、KV Cache 和 workspace。

W7900（48 GB）：单卡跑通了 Qwen3.8-27B 的 Q4 量化版。MTP=2、8192 上下文时约占 19.2 GiB，65 / 65 个计算层都在 GPU 上；另有约 680 MB embedding 留在系统内存。这套配置在 48 GB 卡上仍有明显余量。

给你一个容量直觉：

- **30B 级模型 Q4 量化** ≈ 20 GB 权重。放进 32 GB 卡后理论上还剩约 10 GB；是否够上下文、KV Cache 和 workspace，要看具体模型与配置。
- **70B 级模型 Q4 量化** ≈ 40–45 GB 权重。塞进 48 GB 卡后余量只剩几个 GB，上下文稍长就可能爆显存。

这就是为什么 32 GB 和 48 GB 是工作站卡的两个关键台阶。

R9700 支持多卡配置。四张 32 GB 卡确实共有 128 GB 聚合显存，但不会自动变成程序眼里的一整块。普通数据并行会在每张卡复制完整模型；要让单卡放不下的模型跨卡，需要张量并行、流水线并行或 FSDP / ZeRO 等分片方案。互联带宽、拓扑和通信库会影响性能。这部分水很深，本篇不展开。
...
|---|---:|---|---:|---|
| A100 | 80 GB HBM2e | MI250X | 2×64 GB HBM2e | AMD 官方竞争参照 |
| H100 | 80 GB HBM3 | MI300X | 192 GB HBM3 | AMD 官方竞争参照 |
| H200 | 141 GB HBM3e | MI325X | 256 GB HBM3e | AMD 官方竞争参照 |
| B200 | 180 GB HBM3e | MI355X | 288 GB HBM3e | AMD 官方竞争参照 |

![数据中心对位一览](images/04-datacenter-map.png)

数据中心 GPU 的选型维度远比消费卡复杂——容量、算力、带宽、卡间互联、软件生态全都是变量。这里只帮你建立"N卡和A卡谁对谁"的基本画面。

> **关于 MI250X 的显存：** 表里写的 2×64 GB 不是笔误。一个 OAM 内含两个 GCD，每个连接 64 GB HBM2e；ROCm 将其枚举为两个 GPU 设备，并不是一整块 128 GB 显存池。这个设计在实际使用中有影响。

## 统一内存：都是 128 GB，但完全不是一回事

主流消费级独显常见 16 GB、24 GB，工作站卡能到 48 GB 甚至更高。另一条扩大本地可用容量的路，是让 CPU 和 GPU 共享一整块大内存，不再区分"显存"和"内存"。这就是统一内存方案，AMD 和 NVIDIA 各出了一个：
...
两个方案解决的是同一个问题：给本地运行 70B 甚至更大模型提供更大的可用内存。专业大显存独显和多卡同样能跑大模型，只是硬件形态和软件路径不同。
