# 手把手教你在 AMD Radeon PRO W7900 上丝滑运行 MiniMax H3+ComfyUI 音视频生成

最近我看 minimax H3 视频大模型在知乎上非常受欢迎啊，作为A卡玩家，在 ComfyUI 这个几乎被 CUDA 生态绝对统治的圈子里，好像没有看到几个人在A卡上玩minimax H3，那本文我就来抛砖引玉，希望能够给广大A卡玩家带来一些启发，让大家在A卡上也能好好玩耍，告别吐槽与踩坑。

MiniMax H3 是最近刚杀入 ComfyUI 的重量级模型。它的核心地方在于**原生音视频同步生成**，一次推理，画面和 32kHz 双声道音效同时吐出，而不是先出片再后期配音。这跟之前那些只管画面、声音另想办法的视频模型截然不同。

但是，这东西吧，极度吃显存，而我们上篇文章才给大家发了Radeon Pro W7900，当然是希望大家能够把 H3 这条端到端的音视频生成链路在Radeon Pro W7900能够丝滑跑起来，本文手把手教你怎么跑起来，在这次测试中，W7900 GPU用 276.93 秒生成了一段 5.17 秒的视频：一台红黑色机器人站在计算实验室工作站前，设备灯光亮起，机器人转身看向镜头。输出 864×480、24 FPS，带 32kHz 双声道音频。画面和声音全部来自同一条本地工作流，所有计算发生在这一张A卡上，没有调用任何在线服务。

大家如果有，可以自己试一下，如果没有，可以私我，反正我自己跑完之后最大的感受是，AMD 的 AI 软件栈现在相比几年前真的能用了，ROCm、PyTorch、ComfyUI 三层串通之后，在 A 卡上跑一个刚开放的多模态模型，踩的坑比预想中少得多。

接下来的内容从模型和工作流就位之后开始，直接进运行过程和生成结果。环境搭建（Linux、ROCm、PyTorch、ComfyUI 的从零配置）本篇不展开，如有兴趣，可参考我的过往文章。
...
这次跑通 MiniMax H3 之后，我比较确定一件事：AMD GPU 做本地 AI 生成，已经不是"能不能跑"的问题了。一个刚开放的全模态模型，单张 W7900，从文字到带声音的视频，完整链路跑通，四分半钟出片。

如果你手上有 W7900 或者其他 48GB 以上的 AMD 卡，现在就可以照着这套配置试。工作流和模型都是公开的，换个提示词就能跑自己的东西。

不过，四分半钟显然不是终点。这次的瓶颈非常集中——九成时间都卡在扩散采样阶段。这意味着只要在这一段找到加速手段，总耗时还有很大的压缩空间。下一篇，我们会在同一张 W7900 上动手：换一种注意力计算方式，看四分半能不能再砍一刀。
