...
提到本地部署最新的 AI 视频大模型，绝大多数人的第一反应往往是：“我需要几张 4090？”在 ComfyUI 这个几乎被 NVIDIA CUDA 生态绝对统治的圈子里，A 卡玩家似乎永远在“等适配”和“踩坑”的路上。

但今天，我们要来点硬核的：打破 N 卡的绝对主场，在一张 AMD 显卡上驯服最新的“全模态巨兽”。

MiniMax H3 是最近刚杀入 ComfyUI 的重量级模型。它的杀手锏在于**原生音视频同步生成**——一次推理，画面和 32kHz 双声道音效同时吐出，而不是先出片再后期配音。这跟之前那些只管画面、声音另想办法的“哑巴”视频模型完全是两个物种。

面对这样一个极度吃显存的庞然大物，最核心的问题来了：一张 48GB 显存的 AMD Radeon PRO W7900，能不能把 H3 这条端到端的音视频生成链路完整跑下来？

答案是：不仅能，而且十分顺利。这次测试中，W7900 用 276.93 秒生成了一段 5.17 秒的视频：一台红黑色机器人站在计算实验室工作站前，设备灯光亮起，机器人转身看向镜头。输出 864×480、24 FPS，带 32kHz 双声道音频。画面和声音全部来自同一条本地工作流——所有计算发生在这一张卡上，没有调用任何在线服务。
